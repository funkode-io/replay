...
**Status:** superseded by [ADR-0026](0026-a-policy-tracks-its-position-per-stream.md), which removes contiguity from the feed altogether: a Policy reads each stream over that stream's own sequence, which has no holes to be contiguous across. What survives is the half this ADR is named for — **the filter decides delivery only, never how far a Policy gets** — now applied to one stream's events rather than to a window of positions.
