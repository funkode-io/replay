...
**Status:** accepted, narrowed by [ADR-0024](0024-a-policy-tracks-its-position-per-stream.md): a Policy's feed is no longer one of the reads this ADR governs. It reads one stream at a time, ordered by that stream's own sequence. Hydration, projections and every other read still order by `global_position`.
