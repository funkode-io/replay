...
[ADR-0021](0021-policy-feed-reads-below-the-commit-watermark.md): the [Policy feed] is no
...
every earlier writer finished?", which a position cannot answer, so ADR-0021 moved it to
...
- **Order by commit visibility (`xid8`).** The right key for the feed's "has every earlier
  ...
  (ADR-0021). It answers a different question from this one: a point-in-time read of a
...
- **Gaps do not matter.** A burned position leaves a hole; relative order is unaffected,
  ...
  (ADR-0015) until ADR-0021 left it with no holes to have.
