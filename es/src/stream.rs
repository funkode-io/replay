...
///
/// The embedded scope may itself be scoped, so scopes nest to any depth:
/// `urn:watchlist:main@user:0x78@wallet-type:evm`. The grammar is
///
/// ```text
/// scoped := urn:<nid>:<own-nss-without-@>@<scoped>
/// ```
///
/// which is unambiguous because [`Self::at`] refuses to scope a URN that is
/// already scoped: **an aggregate's own NSS never contains `@`, so the left-most
/// `@` is always the outermost scope boundary**. Every decomposition therefore
/// splits on the *first* `@`, and [`Self::extract_scope`] peels exactly one level,
/// returning a URN that may be peeled again.
///
...
///
/// // nesting: a scope may itself be scoped, and peels one level at a time
/// let region: RegionUrn = RegionUrn::new("uk")?;
/// let branch_in_region: BranchUrn = branch.at(&region)?;      // urn:branch:london@region:uk
/// let deep: BankAccountUrn = account.at(&branch_in_region)?;
/// // urn:bank-account:acct-1@branch:london@region:uk
///
/// let branch: BranchUrn = deep.extract_scope::<BranchUrn>()?;  // still scoped by region
/// let region: RegionUrn = branch.extract_scope::<RegionUrn>()?;
/// ```
pub trait ScopedUrn: Sized + Clone + Into<Urn> + TryFrom<Urn, Error: std::fmt::Debug> {
    ...
    ///
    /// `other` **may** itself be scoped — that is how scopes nest
    /// (`urn:watchlist:main@user:0x78@wallet-type:evm`).
    ///
    /// Fails if the *current* URN is already scoped (NSS contains `@`). That
    /// precondition is what keeps decomposition unambiguous: the left-most `@` is
    /// always the outermost scope boundary, which is the invariant
    /// [`Self::extract_scope`] and [`Self::unscoped`] rely on. To re-scope an
    /// already-scoped URN, call [`Self::unscoped`] first.
    #[track_caller]
    ...
    ///
    /// Peels exactly **one** level: the NSS is split on its first `@`, so when the
    /// scope is itself scoped the returned URN is still scoped and can be peeled
    /// again (`urn:watchlist:main@user:0x78@wallet-type:evm` yields
    /// `urn:user:0x78@wallet-type:evm`).
    ///
        ...
        O: TryFrom<Urn, Error: std::fmt::Debug>,
    {
        ...
        // Split on the *first* '@': `at` guarantees an aggregate's own NSS never
        // contains one, so the left-most '@' is the outermost scope boundary. The
        // scope part may contain further '@' — that is a nested scope, and it is
        // carried through untouched for the caller to peel in turn.
    ...
    ///
    /// Fails if `self` is not scoped (no `@` in the NSS). A nested scope is fine:
    /// the base is everything before the *first* `@`, so
    /// `urn:watchlist:main@user:0x78@wallet-type:evm` unscopes to
    /// `urn:watchlist:main` in one step, whatever the scope's depth.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    ...
    fn unscoped(&self) -> crate::Result<Self> {
        ...
        // Everything before the first '@' is the base URN's own NSS; any further
        // '@' belongs to a nested scope and is dropped along with the rest.
...
pub trait WithId: Sized {
    ...
    /// `StreamId`, and it may itself be scoped (scopes nest). Fails if *this* stream's URN
    /// is already scoped (NSS contains `@`).
    ///
    ...
    ///
    /// Peels one level; a nested scope comes back still scoped.
    ///
...
mod tests {
    ...
    // tenant urn — validates nid == "tenant"; used as the outer scope in nesting tests
    #[derive(Clone, Serialize, Deserialize, PartialEq, Debug)]
    ...
    fn test_at_accepts_an_already_scoped_scope() {
        // The scope may itself be scoped — that is how scopes nest.
        let product = ProductStream::with_string_id("urn:product:sku123").unwrap();
    ...
    fn test_nested_at_and_extract_scope_round_trip() {
        // Built entirely through the API: product @ (catalog @ tenant).
    ...
    fn test_at_still_refuses_to_scope_an_already_scoped_urn() {
        // The left-hand invariant is what keeps first-'@' parsing unambiguous, so it
        // survives nesting: re-scoping a scoped URN is still refused.
        let product =
            ProductStream::with_string_id("urn:product:sku123@catalog:that@tenant:acme").unwrap();
        let catalog = CatalogUrn(Urn::from_str("urn:catalog:other").unwrap());
    ...
    fn test_unscoped_drops_a_nested_scope_whole() {
        // urn:product:sku123@catalog:that@tenant:acme — everything from the first '@'
        // is scope, however deep it nests.
        let product =
            ProductStream::with_string_id("urn:product:sku123@catalog:that@tenant:acme").unwrap();
        let unscoped: Urn = product.unscoped().unwrap().get_id().clone().into();
        assert_eq!(unscoped.to_string(), "urn:product:sku123");
