...
All `#[derive(Urn)]` types get the full set of constructor and accessor methods. The namespace
is either the explicit `#[urn(namespace = "...")]` value or auto-derived from the type name.
`define_aggregate!` also uses `#[derive(Urn)]` internally, so aggregate URN types get these
methods too.
...
| `XxxUrn::namespace()` | Returns the configured NID as a `&'static str`. |
| `.nid()` | NID of this instance (same as `namespace()` for instances that pass construction). |
...
// BankAccountUrn auto-derives namespace "bank-account"
#[derive(Clone, Debug, Urn)]
pub struct BankAccountUrn(Urn);

// Plain ID — namespace is prepended automatically
let id = BankAccountUrn::new("acct-1").unwrap();
assert_eq!(id.to_string(), "urn:bank-account:acct-1");

// Full URN string — namespace is validated
let id = BankAccountUrn::new("urn:bank-account:acct-1").unwrap();
assert_eq!(id.nss(), "acct-1");

// Wrong namespace → error
assert!(BankAccountUrn::new("urn:other:acct-1").is_err());

// Random UUID identifier
let id = BankAccountUrn::new_random();
assert_eq!(id.nid(), "bank-account");

// Accessors
let id = BankAccountUrn::new("acct-42").unwrap();
assert_eq!(id.nid(), "bank-account");
assert_eq!(id.nss(), "acct-42");
...
`Into<Urn> + TryFrom<Urn> + Clone` — which includes all URN types generated by `define_aggregate!`
or `#[derive(Urn)]`. You call it directly on the URN value, with no stream wrapper needed.
...
`at` accepts any type that is `Into<Urn>`, which includes every generated URN type.
