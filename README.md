...
functions (`pg_current_snapshot`, `pg_snapshot_xmin`, `pg_snapshot_xmax`) the policy
feed needs to tell an append that is still in flight from a sequence value that was
burned by an aborted transaction and can therefore never appear. Without them the feed
has no way to distinguish the two, and a single burned position stops every policy
indefinitely ([#164](https://github.com/funkode-io/replay/issues/164)).
